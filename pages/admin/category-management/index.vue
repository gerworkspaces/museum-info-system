<template>
  <div class="w-full">
    <div class="header-page p-6 border-b-2 border-solid">
      <h1 class="text-xl font-bold">Category Management</h1>
    </div>
    <div class="container mx-auto mt-10">
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, categoryList.length) }}
        ...
        >
          ...
          <div class="mt-4">
            ...
            <label class="block mt-4">
              <span>Category ID (Cannot be changed):</span>
              <input
                v-model="selectedCategory.category_id"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
                disabled
              />
            </label>
...
import type { CategoriesResponse, CategoriesRequest } from "~/types/categories";
...
const paginatedCategories = computed(() => {
  ...
  return categoryList.value.slice(start, start + itemsPerPage);
});
const totalPages = computed(() => {
  return Math.ceil(categoryList.value.length / itemsPerPage);
});

const showModal = ref(false);
const showDeleteModal = ref(false);
const showAddModal = ref(false);
const selectedCategory = ref<CategoriesResponse>({
  category_id: "",
  category_name: "",
});
const categoryToDelete = ref<CategoriesResponse>({
  category_id: "",
  category_name: "",
});
const newCategory = ref<CategoriesRequest>({
  category_id: "",
  category_name: "",
});

const openEditModal = (category: CategoriesResponse) => {
...
const openDeleteModal = (category: CategoriesResponse) => {
...
const openAddModal = () => {
  newCategory.value = { category_id: "", category_name: "" }; // Reset new category
...
const deleteCategory = async () => {
  await categoriesStore.deleteCategory(categoryToDelete.value.category_id);
  await categoriesStore.fetchAllCategories();
  closeDeleteModal();
};
