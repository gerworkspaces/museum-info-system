    ...
    <div class="container mx-auto mt-10">
            ...
            <div class="overflow-hidden border rounded-lg border-[#1D253B]">
              <table class="table-auto min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  <tr
                    v-for="payment in paginatedPayment"
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md"
          ...
          {{ Math.min(currentPage * itemsPerPage, paymentList.length) }} of
          {{ paymentList.length }}</span
        >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            ...
            <strong>{{ payment?.event_name }}</strong> không?
          </p>
        </div>
...
const paymentStore = useEventTicketsStore();

const itemsPerPage = 6;
const currentPage = ref(1);

const paymentList = ref<PaymentResponse[]>([]);
const paginatedPayment = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return paymentList.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(paymentList.value.length / itemsPerPage);
});
...
const openDeleteModal = (event: PaymentResponse) => {
  payment.value = event;
...
const deletePayment = async () => {
  const resMess = await paymentStore.deletePayment(payment.value.event_id);
