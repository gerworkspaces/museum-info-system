<template>
  <div class="w-full">
    <div class="header-page p-6 border-b-2 border-solid">
      <h1 class="text-xl font-bold">Post Type Management</h1>
    </div>
    <div class="container mx-auto mt-10">
            ...
            <div class="overflow-hidden border rounded-lg border-[#1D253B]">
              <table class="table-auto min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  <tr
                    v-for="type in paginatedPostTypes"
                    :key="type.post_type_id"
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, postTypeList.length) }}
          of {{ postTypeList.length }}
        </span>
      ...
      >
        ...
        <div
          ...
          @click.stop
        >
          <h3 class="text-lg font-semibold text-white">Edit Post Type</h3>
          <div class="mt-4">
            <label class="block">
              <span>Post Type Name:</span>
              <input
                v-model="selectedPostType.post_type_name"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
            </label>
            <label class="block mt-4">
              <span>Post Type ID (Cannot be changed):</span>
              <input
                v-model="selectedPostType.post_type_id"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
                disabled
              />
            </label>
        ...
        >
          ...
          <div class="mt-4">
            <p class="text-white">
              ...
              <strong>{{ postTypeToDelete?.post_type_name }}</strong
              >?
            </p>
          </div>
          <div class="mt-6 flex justify-end">
      ...
      >
        ...
        <div
          ...
          @click.stop
        >
          <h3 class="text-lg font-semibold text-white">Add New Post Type</h3>
          <div class="mt-4">
            <label class="block">
              <span>Post Type ID:</span>
              <input
                v-model="newPostType.post_type_id"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
            </label>
            <label class="block mt-4">
              <span>Post Type Name:</span>
              <input
                v-model="newPostType.post_type_name"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
            </label>
          </div>
...
import type { PostTypesResponse, PostTypesRequest } from "~/types/post-type";
...
const postTypeList = ref<PostTypesResponse[]>([]);
const paginatedPostTypes = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return postTypeList.value.slice(start, start + itemsPerPage);
});
const totalPages = computed(() => {
  return Math.ceil(postTypeList.value.length / itemsPerPage);
});

const showModal = ref(false);
const showDeleteModal = ref(false);
const showAddModal = ref(false);
const selectedPostType = ref<PostTypesResponse>({
  post_type_id: "",
  post_type_name: "",
});
const postTypeToDelete = ref<PostTypesResponse>({
  post_type_id: "",
  post_type_name: "",
});
const newPostType = ref<PostTypesRequest>({
  post_type_id: "",
  post_type_name: "",
});

const openEditModal = (type: PostTypesResponse) => {
...
const openDeleteModal = (type: PostTypesResponse) => {
  postTypeToDelete.value = type;
...
const openAddModal = () => {
  newPostType.value = { post_type_id: "", post_type_name: "" }; // Reset new post type
...
const addPostType = async () => {
  await postsStore.addPostType(newPostType.value);
  await postsStore.fetchAllPostTypes();
  closeAddModal();
};

const confirmEdit = async () => {
  await postsStore.updatePostType(
    selectedPostType.value,
    selectedPostType.value.post_type_id
  );
  await postsStore.fetchAllPostTypes();
  closeModal();
};
...
const deletePostType = async () => {
  await postsStore.deletePostType(postTypeToDelete.value.post_type_id);
  await postsStore.fetchAllPostTypes();
  closeDeleteModal();
};
...
watch(
  () => postsStore.post_types,
  (newPostTypes) => {
    postTypeList.value = newPostTypes;
  }
);
</script>
