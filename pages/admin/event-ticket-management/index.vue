<template>
  <div class="w-full">
    <div class="header-page p-6 border-b-2 border-solid">
      <h1 class="text-xl font-bold">Event Tickets Management</h1>
    </div>
    <div class="container mx-auto mt-10">
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, ticketList.length) }} of
          {{ ticketList.length }}
        </span>
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Edit Ticket</h3>
        <div class="mt-4">
          <label class="block">
            <span>Event:</span>
            <select
              v-model="selectedTicket.event_id"
          ...
          <label class="block">
            ...
            <input
              v-model="selectedTicket.ticket_name"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            <span>Price:</span>
            <input
              v-model="selectedTicket.ticket_price"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            ...
            <input
              v-model="selectedTicket.total_tickets"
              type="number"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            ...
            <input
              v-model="selectedTicket.available_tickets"
              type="number"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            <span>Ticket Picture:</span>
            <input
              type="file"
              multiple
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn lưu thay đổi cho vé
            <strong>{{ selectedTicket?.ticket_name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            ...
            <strong>{{ ticketToDelete?.ticket_name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Add New Ticket</h3>
        <div class="mt-4">
          <label class="block">
            <span>Event:</span>
            <select
          ...
          <label class="block mt-4">
            <span>Price:</span>
            <input
              v-model="newTicket.ticket_price"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            <span>Total Tickets:</span>
            <input
              v-model="newTicket.total_tickets"
              type="number"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            <span>Available Tickets:</span>
            <input
              v-model="newTicket.available_tickets"
              type="number"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
...
import type { TicketRequest, TicketsResponse } from "~/types/event-tickets";
...
const ticketList = ref<TicketsResponse[]>([]);
const eventList = ref<EventResponse[]>([]);
const availableEvents = ref<EventResponse[]>([]);
const paginatedTickets = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return ticketList.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(ticketList.value.length / itemsPerPage);
});

const showModal = ref(false);
const selectedTicket = ref<TicketsResponse>({
  event_id: 0,
  ticket_name: "",
  ticket_price: "",
  total_tickets: 0,
...
const ticketToDelete = ref<TicketsResponse>({
  event_id: 0,
  ticket_name: "",
  ticket_price: "",
  total_tickets: 0,
...
const newTicket = ref<TicketRequest>({
  event_id: 0,
  ticket_name: "",
  ticket_price: "",
  total_tickets: 0,
...
const openEditModal = (ticket: TicketsResponse) => {
...
const openDeleteModal = (ticket: TicketsResponse) => {
...
const saveTicket = async () => {
  if (files.value.length !== 0) {
    ...
    if (response === "success!") {
      // for single file
      // selectedTicket.value.ticket_image = files.value[0].name;
      // for multiple files
      selectedTicket.value.ticket_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await eventTicketsStore.updateTicket(selectedTicket.value);
  await eventTicketsStore.getAllTickets();
  console.log(resMess);
  closeConfirmModal();
  closeModal();
};
...
const deleteTicket = async () => {
  const resMess = await eventTicketsStore.deleteTicket(
    ticketToDelete.value.event_id
  );
  await eventTicketsStore.getAllTickets();
  console.log(resMess);
  closeDeleteModal();
...
const addTicket = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // newTicket.value.ticket_image = files.value[0].name;
      // for multiple files
      newTicket.value.ticket_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await eventTicketsStore.addTicket(newTicket.value);
  await eventTicketsStore.getAllTickets();
  console.log(resMess);
  closeAddModal();
};
...
watch(
  () => eventTicketsStore.tickets,
  (newTickets) => {
    ticketList.value = newTickets;
  }
);
watch(
  () => eventStore.events,
  (newEvents) => {
    ...
    availableEvents.value = eventList.value.filter(
      (event) =>
        !ticketList.value.some((ticket) => ticket.event_id === event.event_id)
    );
  }
);
</script>
