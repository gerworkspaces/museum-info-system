  ...
  <div class="w-full">
    ...
    <div class="container mx-auto mt-10">
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, roleList.length) }} of
          {{ roleList.length }}
        </span>
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Are you sure you want to save changes for role
            <strong>{{ selectedRole?.role_name }}</strong
            >?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
...
import type { RoleRequest, RoleResponse } from "~/types/roles";
...
const paginatedRoles = computed(() => {
  ...
  return roleList.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(roleList.value.length / itemsPerPage);
});

const showModal = ref(false);
const selectedRole = ref<RoleResponse>({
  role_id: 0,
  role_name: "",
});

const showDeleteModal = ref(false);
const roleToDelete = ref<RoleResponse>({
  role_id: 0,
  role_name: "",
});
...
const saveRole = async () => {
  const resMess = await rolesStore.updateRole(
    selectedRole.value,
...
const deleteRole = async () => {
  const resMess = await rolesStore.deleteRole(roleToDelete.value.role_id);
  await rolesStore.fetchAllRoles();
  console.log(resMess);
  closeDeleteModal();
};
...
const addRole = async () => {
  const resMess = await rolesStore.addRole(newRole.value);
  await rolesStore.fetchAllRoles();
  console.log(resMess);
  closeAddModal();
};
