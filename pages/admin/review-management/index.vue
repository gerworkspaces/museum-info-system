    ...
    <div class="container mx-auto mt-10">
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, reviewList.length) }}
          of {{ reviewList.length }}
        </span>
        ...
        >
          ...
          <div class="mt-4">
            <p class="text-white">
              ...
              <strong>{{ reviewToDelete?.content }}</strong> không?
            </p>
          </div>
...
import type { ReviewEventResponse } from "~/types/event";
...
const reviewStore = useEventStore();

const reviewList = ref<ReviewEventResponse[]>([]);

const currentPage = ref(1);
const itemsPerPage = ref(10);
const totalPages = computed(() =>
  Math.ceil(reviewList.value.length / itemsPerPage.value)
);
const paginatedReviews = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  return reviewList.value.slice(start, start + itemsPerPage.value);
});

const showDeleteModal = ref(false);
const reviewToDelete = ref<ReviewEventResponse | null>(null);

const openDeleteModal = (feedback: ReviewEventResponse) => {
  reviewToDelete.value = feedback;
...
const deleteReview = async () => {
  if (reviewToDelete.value) {
    await reviewStore.deleteReviewEvent(reviewToDelete.value.review_id!);
    await reviewStore.getAllReviews();
    closeDeleteModal();
  }
};
