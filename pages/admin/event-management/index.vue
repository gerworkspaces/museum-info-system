  ...
  <div class="w-full">
    ...
    <div class="container mx-auto mt-10">
                ...
                <tbody class="divide-y divide-gray-300">
                  ...
                  >
                    ...
                    >
                      {{
                        museumList.find(
                          (museum) => museum.museum_id === event.museum_id
                        )?.name || "N/A"
                      }}
                    </td>
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md"
          ...
          {{ Math.min(currentPage * itemsPerPage, eventList.length) }} of
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Edit Event</h3>
        <div class="mt-4">
          <label class="block">
          ...
          <label class="block mt-4">
            <span>Museum:</span>
            <select
              v-model="selectedEvent.museum_id"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            >
              <option
                v-for="museum in museumList"
                :key="museum.museum_id"
                :value="museum.museum_id"
              >
                {{ museum.name }}
              </option>
            </select>
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn lưu thay đổi cho sự kiện
            <strong>{{ selectedEvent?.event_name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            ...
            <strong>{{ eventToDelete?.event_name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Add New Event</h3>
        <div class="mt-4">
          <label class="block">
            <span>Event Name:</span>
            <input
...
const paginatedEvents = computed(() => {
  ...
  return eventList.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(eventList.value.length / itemsPerPage);
});

const showModal = ref(false);
const selectedEvent = ref<EventRequest>({
  event_id: 0,
  event_name: "",
  event_date: "",
  event_image: "",
...
const newEvent = ref<EventRequest>({
  event_id: 0,
  event_name: "",
  event_date: "",
  event_image: "",
...
const openEditModal = (event: EventRequest) => {
  selectedEvent.value = { ...event, event_date: formatDate(event.event_date) };
...
const openDeleteModal = (event: EventRequest) => {
...
const saveEvent = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // selectedEvent.value.event_image = files.value[0].name;
      // for multiple files
      selectedEvent.value.event_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await eventStore.updateEvent(
...
const deleteEvent = async () => {
  const resMess = await eventStore.deleteEvent(eventToDelete.value.event_id);
  await eventStore.fetchEvents();
  console.log(resMess);
  closeDeleteModal();
};
...
const addEvent = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // selectednewEventEvent.value.event_image = files.value[0].name;
      // for multiple files
      newEvent.value.event_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await eventStore.addEvent(newEvent.value);
  await eventStore.fetchEvents();
  console.log(resMess);
  closeAddModal();
};
...
onMounted(async () => {
  await eventStore.fetchEvents();
  await museumStore.fetchAllMuseums();
});
