    ...
    <div class="container mx-auto mt-10">
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, feedbackList.length) }}
        ...
        >
          ...
          <div class="mt-4">
            <p class="text-white">
              ...
              <strong>{{ feedbackToDelete?.content }}</strong> không?
            </p>
          </div>
...
const totalPages = computed(() =>
  Math.ceil(feedbackList.value.length / itemsPerPage.value)
);
const paginatedFeedbacks = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  return feedbackList.value.slice(start, start + itemsPerPage.value);
});

const showDeleteModal = ref(false);
const feedbackToDelete = ref<FeedbackResponse | null>(null);

const openDeleteModal = (feedback: FeedbackResponse) => {
...
const deleteFeedback = async () => {
  if (feedbackToDelete.value) {
    await feedbackStore.deleteFeedback(feedbackToDelete.value.feedback_id!);
...
watch(
  ...
  (newFeedbacks) => {
    feedbackList.value = newFeedbacks;
  }
);
watch(
  () => userStore.users,
