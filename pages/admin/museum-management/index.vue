<template>
  <div class="w-full">
    <div class="header-page p-6 border-b-2 border-solid">
      <h1 class="text-xl font-bold">Museum Management</h1>
    </div>
    <div class="container mx-auto mt-10">
      <div class="my-8">
        <button
          @click="openAddModal"
          class="px-4 py-2 rounded-md bg-green-700 text-white flex justify-center items-center gap-2"
        >
          <Icon icon="iconamoon:sign-plus-circle" width="24" height="24" />
          Add Museum
        </button>
      </div>
      <div class="flex flex-col">
        <div class="overflow-x-auto pb-4">
          <div class="min-w-full inline-block align-middle">
            <div class="overflow-hidden border rounded-lg border-[#1D253B]">
              <table class="table-auto min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  <tr
                    ...
                    class="bg-[#525F7F] transition-all duration-500 hover:bg-[#7b8dba]"
                  >
                    <td
                      class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-white"
                    >
                      {{ museum.museum_id }}
                    </td>
                    <td
                    ...
                    >
                      {{
                        categoryList.find(
                          (category) =>
                            category.category_id === museum.category_id
                        )?.category_name || "N/A"
                      }}
                    </td>
                    <td
                      class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-white"
                    >
                      {{ `${museum.open_time} - ${museum.close_time}` }}
                    </td>
                    <td class="flex p-5 items-center gap-0.5">
                      <button
                        @click="openEditModal(museum)"
                      ...
                      >
                        ...
                        >
                          <path
                            class="fill-indigo-500 group-hover:fill-white"
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          {{ (currentPage - 1) * itemsPerPage + 1 }} -
          {{ Math.min(currentPage * itemsPerPage, museumList.length) }}
          of {{ museumList.length }}
        </span>
        <!-- Phân trang -->
    ...
    >
      ...
      ></div>
      <div
        class="relative z-20 w-full max-w-lg p-6 bg-[#1D253B] rounded-lg"
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Edit Museum</h3>
        <div class="mt-4">
          <label class="block">
            <span class="">Museum Name:</span>
            <input
              v-model="selectedMuseum.name"
              type="text"
          ...
          <label class="block mt-4">
            ...
            <input
              v-model="selectedMuseum.location"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            ...
            <textarea
              v-model="selectedMuseum.description"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            ></textarea>
          </label>
          <label class="block mt-4">
            ...
            <select
              v-model="selectedMuseum.category_id"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            >
          ...
          <label class="block mt-4">
            ...
            <div class="flex">
              <input
                v-model="selectedMuseum.open_time"
                type="time"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
              <span class="mx-2"> - </span>
              <input
                v-model="selectedMuseum.close_time"
                type="time"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
            </div>
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Confirm Edit</h3>
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn lưu thay đổi cho bảo tàng
            <strong>{{ selectedMuseum?.name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn xóa bảo tàng
            <strong>{{ museumToDelete?.name }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Add New Museum</h3>
        <div class="mt-4">
          <label class="block">
            <span class="">Museum Name:</span>
            <input
              v-model="newMuseum.name"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          ...
          <label class="block mt-4">
            ...
            <div class="flex">
              <input
                v-model="newMuseum.open_time"
                type="time"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
              <span class="mx-2"> - </span>
              <input
                v-model="newMuseum.close_time"
                type="time"
                class="mt-1 block w-full outline-none p-2 rounded-md text-black"
              />
            </div>
...
import type { MuseumsRequest, MuseumsResponse } from "~/types/museums";
...
const museumList = ref<MuseumsResponse[]>([]);
const categoryList = ref<CategoriesResponse[]>([]);

const paginatedMuseums = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage;
  return museumList.value.slice(start, start + itemsPerPage);
});

const totalPages = computed(() => {
  return Math.ceil(museumList.value.length / itemsPerPage);
});

const showModal = ref(false);
const selectedMuseum = ref<MuseumsResponse>({
  museum_id: 0,
  name: "",
  location: "",
  description: "",
...
const museumToDelete = ref<MuseumsResponse>({
  museum_id: 0,
  name: "",
  location: "",
  description: "",
...
const newMuseum = ref<MuseumsRequest>({
  name: "",
  location: "",
  description: "",
  category_id: "",
...
const openEditModal = (museum: MuseumsResponse) => {
...
const openDeleteModal = (museum: MuseumsResponse) => {
  museumToDelete.value = museum;
...
const saveMuseum = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // selectedMuseum.value.museum_image = files.value[0].name;
      // for multiple files
      selectedMuseum.value.museum_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await museumsStore.updateMuseum(
    selectedMuseum.value,
    selectedMuseum.value.museum_id
  );
...
const deleteMuseum = async () => {
  console.log(museumToDelete.value);
  const resMess = await museumsStore.deleteMuseum(
    museumToDelete.value.museum_id
  );
  await museumsStore.fetchAllMuseums();
  console.log(resMess);
...
const addMuseum = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // newMuseum.value.museum_image = files.value[0].name;
      // for multiple files
      newMuseum.value.museum_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await museumsStore.addMuseum(newMuseum.value);
  await museumsStore.fetchAllMuseums();
  console.log(resMess);
