  ...
  <div class="w-full">
              ...
              <table class="table-auto min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  ...
                  >
                    ...
                    >
                      {{
                        rolesList.find((role) => role.role_id === user.role_id)
                          ?.role_name || "N/A"
                      }}
                    </td>
                    <td
      ...
      >
        ...
        <div class="mt-4">
          ...
          <label class="block mt-4">
            <span>Address:</span>
            <input
              v-model="selectedUser.address_info"
              type="text"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            <span>Password:</span>
            <input
              v-model="selectedUser.password_hash"
              type="password"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn lưu thay đổi cho người dùng
...
const userToDelete = ref<UserResponse>({
  user_id: 0,
  username: "",
  email: "",
  phone: "",
...
const saveUser = async () => {
  if (files.value.length !== 0) {
    ...
    if (response === "success!") {
      // for single file
      selectedUser.value.image = files.value[0].name || "";
      // for multiple files
  ...
  const resMess = await userStore.updateUser(
    selectedUser.value,
...
const deleteUser = async () => {
  ...
  const resMess = await userStore.deleteUser(userToDelete.value.user_id);
  await userStore.fetchAllUsers();
  console.log(resMess);
  closeDeleteModal();
};
...
const addUser = async () => {
  if (files.value.length !== 0) {
    ...
    if (response === "success!") {
      // for single file
      newUser.value.image = files.value[0].name || "";
      // for multiple files
      // selectedUser.value.image =
      //   files.value.length === 1
      //     ? files.value[0].name
  ...
  const resMess = await userStore.register(newUser.value);
  await userStore.fetchAllUsers();
  console.log(resMess);
  closeAddModal();
};
