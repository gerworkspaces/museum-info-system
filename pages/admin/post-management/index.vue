<!-- pages/admin/post-management/index.vue -->
<template>
  <div class="w-full">
    ...
    <div class="container mx-auto mt-10">
              ...
              <table class="table-auto min-w-full rounded-xl">
                ...
                <tbody class="divide-y divide-gray-300">
                  ...
                  >
                    ...
                    >
                      {{
                        userList.find((user) => user.user_id === post.user_id)
                          ?.username || "N/A"
                      }}
                    </td>
                    <td
                    ...
                    >
                      {{
                        postTypeList.find(
                          (type) => type.post_type_id === post.post_type_id
                        )?.post_type_name || "N/A"
                      }}
                    </td>
                    <td
      ...
      <div class="flex justify-between mt-4">
        <span class="px-4 py-2 bg-[#525F7F] rounded-md">
          ...
          {{ Math.min(currentPage * itemsPerPage, postList.length) }}
          of {{ postList.length }}
        </span>
        <!-- Phân trang -->
      ...
      >
        ...
        <div class="mt-4">
          ...
          <label class="block mt-4">
            ...
            <input
              v-model="selectedPost.post_date"
              type="date"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            />
          </label>
          <label class="block mt-4">
            ...
            <select
              v-model="selectedPost.post_type_id"
              class="mt-1 block w-full outline-none p-2 rounded-md text-black"
            >
              <option
                v-for="type in postTypeList"
    ...
    >
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            Bạn có chắc chắn muốn lưu thay đổi cho bài viết
            <strong>{{ selectedPost?.content }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
      ...
      >
        ...
        <div class="mt-4">
          <p class="text-white">
            ...
            <strong>{{ postToDelete?.content }}</strong> không?
          </p>
        </div>
        <div class="mt-6 flex justify-end">
          <button
    ...
    >
      ...
      <div
        ...
        @click.stop
      >
        <h3 class="text-lg font-semibold text-white">Add New Post</h3>
        <div class="mt-4">
          <label class="block">
            <span class="">Title:</span>
            <input
...
import type { PostsRequest, PostsResponse } from "~/types/posts";
import { Icon } from "@iconify/vue";
import { useUserStore } from "~/stores/user-store";
import type { UserResponse } from "~/types/user";
import type { PostTypesResponse } from "~/types/post-type";
...
const postList = ref<PostsResponse[]>([]);
...
const totalPages = computed(() => {
  return Math.ceil(postList.value.length / itemsPerPage);
});

const showModal = ref(false);
const selectedPost = ref<PostsResponse>({
  post_id: 0,
  user_id: 0,
  title: "",
  content: "",
...
const postToDelete = ref<PostsResponse>({
  post_id: 0,
  user_id: 0,
  content: "",
  post_date: "",
...
const newPost = ref<PostsRequest>({
...
const openEditModal = (post: PostsResponse) => {
  selectedPost.value = {
    ...post,
...
const openDeleteModal = (post: PostsResponse) => {
...
const savePost = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      // for single file
      // selectedPost.value.post_image = files.value[0].name;
      // for multiple files
      selectedPost.value.post_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await postsStore.updatePost(
    selectedPost.value,
...
const deletePost = async () => {
  const resMess = await postsStore.deletePost(postToDelete.value.post_id);
  await postsStore.fetchAllPosts();
  console.log(resMess);
  closeDeleteModal();
};
...
const addPost = async () => {
  if (files.value) {
    ...
    if (response === "success!") {
      ...
      // selectedPost.value.post_image = files.value[0].name;
      // for multiple files
      newPost.value.post_image =
        files.value.length === 1
          ? files.value[0].name
          : files.value.map((file) => file.name).join(", ");
    }
  }
  const resMess = await postsStore.addPost(newPost.value);
  await postsStore.fetchAllPosts();
  console.log(resMess);
  closeAddModal();
};
